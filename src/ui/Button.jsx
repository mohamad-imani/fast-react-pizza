...
export default function Button({ type, children, disabled, to, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      ' text-sm inline-block rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 focus:outline-none focus:bg-stone-300 focus:ring focus:ring-stone-200 focus:ring-offset-2 hover:text-stone-800 focus:text-stone-800 disabled:cursor-not-allowed md:px-6 md:py-3.5 px-4 py-2.5 ',
  };
  if (to)
    return (
